<modification>
	<id>Fixed Price - OpenCart 1.4.9.x core file modifications</id>
	...
	<file name="catalog/model/catalog/product.php">
		...
		<operation>
			<search position="replace"><![CDATA[return $query->row;]]></search>
	<add><![CDATA[
	...
	//Sun0703 start
		...
		<operation>
			<search position="replace" index="2"><![CDATA[return $query->rows;]]></search>
	<add><![CDATA[
	$tmprows=$query->rows;
	$newrows=array();
	//Sun0703 start
	if(isset($_SESSION['use_fixed_price'])){
		foreach($tmprows as $k=>$query->row){
		...
		<operation>
			<search position="replace" index="2"><![CDATA[return $query->row['price'];]]></search>
	<add><![CDATA[
	...
	return $query->row['price'];
		...
		<operation>
			<search position="replace"><![CDATA[$product_option_value_data[] = array(]]></search>
	<add><![CDATA[
	...
	//Sun0703 start
		...
		<operation>
			<search position="replace" ><![CDATA[if ($option_value_query->row['prefix'] == '+') ]]></search>
	<add><![CDATA[
	//Sun0703 start
...
// Settings
 $sql = "SELECT * FROM " . DB_PREFIX . "setting where `key`='fixed_prices'";
...
//sun0703 end		
		...
		<operation>
			<search position="after" index="1"><![CDATA[$this->db->query("DELETE FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");]]></search>
	<add><![CDATA[
		...
		<operation>
			<search position="after"><![CDATA[$product_option_value_id = $this->db->getLastId();]]></search>
		<add><![CDATA[
		 //sun0703 start
		 $new_product_option_value_id = $product_option_value_id;
...
//sun0703 end	
	...
	<add><![CDATA[<td>
		...
		<operation>
			<search position="replace"><![CDATA[<input type="text" name="product_option[<?php echo $option_row; ?>][product_option_value][<?php echo $option_value_row; ?>][price]" value="<?php echo $product_option_value['price']; ?>" />]]></search>
		<add><![CDATA[
		 <?php
			  if(isset($_SESSION['use_fixed_price'])){
			  $query2 = $this->db->query("Select * from " . DB_PREFIX . "currency order by value");
		...
		<operation>
			<search position="replace"><![CDATA[html += '<td><input type="text" name="product_option[' + option_id + '][product_option_value][' + option_value_row + '][price]" value="" /></td>';]]></search>
			<add><![CDATA[
			<?php
			  if(isset($_SESSION['use_fixed_price'])){
			  echo 'html += \'<td>\';';
			...
			foreach($query2->rows as $resultc) {
			if($resultc['value']==1)
			echo ' html +=\''.$resultc['code'].'<input type="text" name="product_option[\' + option_id + \'][product_option_value][\' + option_value_row + \'][prices]['.$resultc['code'].']" value=""  onkeyup="document.getElementById(\\\'default_option_price\' + option_row +option_value_row+ \'\\\').value=this.value;" /><br>\';';
			else
			echo ' html +=\''.$resultc['code'].'<input type="text" name="product_option[\' + option_id + \'][product_option_value][\' + option_value_row + \'][prices]['.$resultc['code'].']" value=""  /><br>\';';
	
			 }
			 	
		echo ' html +=\''.'<input type="hidden" name="product_option[\' + option_id + \'][product_option_value][\' + option_value_row + \'][price]" value="" id="default_option_price\' + option_row  +option_value_row + \'" /><br>\';';
			 echo 'html += \'</td>\';';
	...
	if(isset($_SESSION['use_fixed_price'])){
		...
		<operation>
			<search position="replace"><![CDATA[echo $extension['status']]]></search>
	<add><![CDATA[
	if(isset($_SESSION['use_fixed_price'])){
		if($extension['name']=='Fixed Prices')
